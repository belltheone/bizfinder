...
def render_tab_lab():
    ...
    with st.expander("ℹ️ AI 분석 알고리즘 상세 보기 (필수 확인)", expanded=True):
        st.markdown("""
        ### 📖 초보자를 위한 AI 분석 과정 설명서
        저희 AI는 **선생님이 학생의 시험지를 채점하듯이** 다음 4단계로 꼼꼼하게 공고문을 분석합니다.

        #### 1단계: 🚫 자격 요건 확인 (입구 컷)
        > *"우리 회사가 지원할 자격은 되나?"*
        
        가장 먼저 **지원 자격(업력, 매출액, 지역 등)**을 확인합니다. 만약 조건에 맞지 않으면(예: '3년 미만 기업만 가능'한데 우리가 5년 차라면) **'지원 불가(Kill Switch)'** 판정을 내리고 분석을 멈춥니다. 불필요한 시간 낭비를 막아줍니다.

        #### 2단계: 💯 적합성 채점 (100점 만점)
        > *"이 과제가 우리랑 얼마나 잘 맞을까?"*
        
        세 가지 기준을 가지고 꼼꼼하게 점수를 매깁니다.
        - **분야 적합성 (50점)**: "우리 전공인가?" (AI/SW 개발인지, 센서/IoT 하드웨어인지 확인)
        - **사업 매력도 (30점)**: "가성비가 좋은가?" (인건비를 현금으로 주는지, 우리가 주도권을 가질 수 있는지)
        - **기술 일치도 (20점)**: "우리가 잘하는 기술인가?" (Python, 임베디드 등 보유 기술 활용 가능성)

        #### 3단계: 🤝 전략 수립 (혼자? 함께?)
        > *"이거 혼자 할 수 있을까, 팀을 짜야 할까?"*
        
        공고 내용을 분석해 **단독으로 지원**하는 게 좋을지, 아니면 **학교(대학), 연구소, 수요처(병원/공장)와 팀(컨소시엄)**을 꾸려야 유리할지 판단하고 최적의 전략을 제안합니다.
